OC.L10N.register(
    "twofactor_totp",
    {
    "Enable TOTP" : "აამოქმედეთ TOTP",
    "This is your new TOTP secret:" : "ეს თქვენი ახალი TOTP საიდუმლოა:",
    "Scan this QR code with your TOTP app" : "ამ QR კოდის სკანირება თქვენი TOTP აპლიკაციით",
    "Once you have configured your app, enter a test code below to ensure that your app has been configured correctly." : "აპლიკაციის კონფიგურირების შემდეგ, შესამოწმებლად შეიყვანეთ ქვემოთ მოცემული სატესტო კოდი.",
    "Authentication code" : "აუტენნტიფიკაციის კოდი",
    "Verify" : "დამოწმება",
    "Could not verify your key. Please try again" : "თქვენი გასაღები ვერ დამოწმდა. გთხოვთ სცადოთ ახლიდან",
    "You enabled TOTP two-factor authentication for your account" : "თქვენს ანგარიშზე აამოქმედეთ TOTP ორ-ფაქტორიანი აუტენტიფიკაცია",
    "You disabled TOTP two-factor authentication for your account" : "თქვენს ანგარიშზე გათისეთ TOTP ორ-ფაქტორიანი აუტენტიფიკაცია",
    "TOTP (Authenticator app)" : "TOTP (აუტენტიფიკაციის აპლიკაცია)",
    "Authenticate with a TOTP app" : "გაიარეთ აუტენტიფიკაცია თქვენს TOTP აპლიკაციასთან",
    "Two Factor TOTP Provider" : "ორ ფაქტორიანი TOTP მომწოდებელი",
    "Submit" : "გაგზავნა",
    "Get the authentication code from the two-factor authentication app on your device." : "აუტენტიფიკაციის კოდი მიიღეთ თქვენს მოწყობილობაზე არსებული ორ-ფაქტორიანი აუტენტიფიკაციის აპლიკაციისგან.",
    "TOTP second-factor auth" : "TOTP მეორე-ფაქტორის აუტენტიფიკაცია"
},
"nplurals=1; plural=0;");
